...
-h --height: Height of marker to generate in mm (pixels) [does not work if size is given]
...
def SetupArguments(ap):
    ...
    ap.add_argument("-l", "--length", type=int, required=False,
                    help="Width of marker to generate in mm (pixels) [does not work if size is given]")
    ap.add_argument("-h", "--height", type=int, required=False,
...
def CheckSize(size, length, height):
    print(size)
    print(length)
    print(height)
...
def CreateObj(marker_type, num, marker_id, size, output_dir):
    ...
    vertexes_str = CreateObjFileVerteces(size/1000, size/1000)
...
if __name__ == '__main__':
    ...
    match (Function_DICT.get(args["function"], None)):
        ...
        case 1:
            ...
            CheckSize(args["size"], args["length"], args["height"])
        case 2:
